...
public:
  ...
  void SetVertMutRate(double mut) {vt_mut_rate = mut;}
  void SetHorizMutRate(double mut) {ht_mut_rate = mut;}
  ...
  void Update() {
    ...
    for (size_t i : schedule) {
      ...
      if (pop[i]->GetPoints() >= host_repro ) {  // if host has more points than required for repro                                                                                                   
        ...
        host_baby->mutate(random, vt_mut_rate);
        pop[i]->mutate(random, vt_mut_rate); //parent mutates and loses current resources, ie new organism but same symbiont  
        pop[i]->SetPoints(0);
        ...
        for(size_t j = 0; j< (pop[i]->GetSymbionts()).size(); j++){
          ...
          if (WillTransmit()) { //Vertical transmission!  
            ...
            sym_baby->mutate(random, vt_mut_rate);
            parent.mutate(random, vt_mut_rate); //mutate parent symbiont                                   
        ...
        for(size_t j = 0; j < syms.size(); j++){
	      ...
	      data_node_burst_size -> AddDatum(repro_syms.size());
              ...
              for(size_t o=0; o< offspring_per_tick; o++) {
                if(syms[j].GetPoints() >= sym_lysis_res) { //check if sym has resources to produce offspring
                  ...
                  sym_baby->mutate(random, vt_mut_rate);
                  syms[j].mutate(random, vt_mut_rate);
          ...
          if(h_trans) { //non-lytic horizontal transmission enabled
            if (syms[j].GetPoints() >= sym_h_res) {
              ...
              Symbiont *sym_baby = new Symbiont(syms[j].GetIntVal());
              sym_baby->mutate(random, ht_mut_rate);
              syms[j].mutate(random, ht_mut_rate);
